...
// API endpoint to connect two pieces at the given node.
export const connectPiecesAtNode = async (req: Request, res: Response, next: NextFunction) => {
  // matchedData only includes fields defined in the validator middleware for this route
...
// API endpoint to disconnect two pieces at the given node
...
export const movePiece = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    const span = trace.getActiveSpan();
    span?.setAttribute('_.request.type', 'movePiece');
